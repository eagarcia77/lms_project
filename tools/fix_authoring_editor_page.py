    ...
    async def new_item(module_id: int, request: Request):
        ...
        try:
            with db() as conn:
                ...
                item_count = int(count_rows[0].get("total", 0)) if count_rows else 0

            options = "".join(
                f'<option value="{html.escape(kind)}">{html.escape(kind.replace("_", " ").title())}</option>'
            ...
            body = f'''<p><a href="/admin/authoring/courses/{course_id}">← Volver al curso</a></p>
