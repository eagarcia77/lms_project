...
RUN NEXUS_SESSION_SECRET=build-verification-secret-change-in-production python -c "from app.runtime_entry import app; routes=[(getattr(r,'path',''),set(getattr(r,'methods',[]) or [])) for r in app.routes]; required=[('/admin/login','GET'),('/admin/courses','GET'),('/admin/authoring','GET'),('/admin/authoring/courses','POST'),('/admin/authoring/courses/{course_id}','GET'),('/admin/authoring/courses/{course_id}/modules','POST'),('/admin/authoring/courses/{course_id}/ai-plan','POST'),('/admin/authoring/modules/{module_id}/items/new','GET'),('/admin/authoring/modules/{module_id}/items','POST'),('/admin/authoring/modules/{module_id}/google','POST'),('/admin/authoring/modules/{module_id}/odf/{kind}','GET'),('/admin/authoring/items/{item_id}/forum','GET'),('/admin/authoring/items/{item_id}/forum','POST'),('/admin/authoring/items/{item_id}/preview','GET')]; missing=[f'{m} {p}' for p,m in required if not any(rp==p and m in methods for rp,methods in routes)]; print('Course Studio V6 routes:', [f'{"/".join(sorted(m))} {p}' for p,m in routes if p.startswith('/admin/authoring')]); assert not missing, missing"
